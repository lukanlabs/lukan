...
function loadGoogleWorkspaceConfig() {
  // Fallback: read clientId/clientSecret from google-workspace plugin
...
async function authenticate() {
  ...
  const clientId = config.clientId || gwConfig.clientId || process.env.GOOGLE_CLIENT_ID;
  const clientSecret = config.clientSecret || gwConfig.clientSecret || process.env.GOOGLE_CLIENT_SECRET;

  if (!clientId) {
    console.error(
      "\x1b[31mGoogle Client ID not configured.\x1b[0m\n" +
      "Set it via: lukan gmail client_id set \"your-client-id\"\n" +
  ...
  if (!clientSecret) {
    console.error(
      ...
      "Set it via: lukan gmail client_secret set \"your-client-secret\""
    );
    process.exit(1);
  }
  ...
  console.log("\n\x1b[1m\x1b[36m  lukan gmail auth\x1b[0m");
  console.log("\x1b[2m  Gmail authentication (OAuth2 PKCE)\x1b[0m\n");
  ...
  console.log("\x1b[32m✓\x1b[0m Gmail authentication successful!");
  console.log(`\x1b[32m✓\x1b[0m Credentials saved to \x1b[2m${CONFIG_PATH}\x1b[0m`);
  console.log(
    "\n\x1b[2mGmail tools (search, read, send, reply, modify) are now available.\x1b[0m\n"
  );
}
