...
function StepNode({ data }: { data: StepNodeData }) {
  ...
  const tokens = stepRun
    ? stepRun.tokenUsage.input + stepRun.tokenUsage.output
    : 0;
  const hasOutput =
    stepRun && (stepRun.output || stepRun.error) && status !== "pending";
    ...
    >
      ...
      >
        ...
        {status === "running" && (
          <Loader2
            size={11}
            style={{ color: c, animation: "spin 1s linear infinite" }}
          />
        )}
        {status === "success" && (
            ...
            >
              ...
              {hasOutput && (
                <button
                  onClick={() => {
                    setMenuOpen(false);
                    if (stepRun) onViewOutput?.(stepRun);
                  }}
                  style={menuItemStyle}
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = "#2a2a2a";
          ...
          {notifyPlugin ? (
            <>
              ...
              <span style={{ color: "#555" }}>
                {notifyChannel
                  ? `(${notifyChannel.slice(0, 16)}${(notifyChannel?.length ?? 0) > 16 ? "..." : ""})`
                  : "(UI only)"}
              </span>
            </>
          ) : (
        ...
        >
          {isRunning && (
            ...
            >
              ...
              />
              {stepRun.error?.startsWith("[activity]")
                ? stepRun.error.slice(11)
                : "processing..."}
            </span>
          )}
          {stepRun.completedAt && stepRun.startedAt && (
            <span style={{ display: "flex", alignItems: "center", gap: 2 }}>
              <Clock size={8} />{" "}
              {formatElapsed(stepRun.startedAt, stepRun.completedAt)}
            </span>
          )}
          {tokens > 0 && (
            <span style={{ display: "flex", alignItems: "center", gap: 2 }}>
...
function DeletableEdge({
  ...
  const onDelete = (data as Record<string, unknown>)?.onDelete as
    | ((id: string) => void)
    | undefined;
...
function EditStepSheet({
  ...
  const [providers, setProviders] = useState<
    { name: string; currentModel?: string; defaultModel: string }[]
  >([]);
  ...
  // Approval config
  const [notifyPlugin, setNotifyPlugin] = useState(
    step.approval?.notifyPlugin ?? "",
  );
  ...
  useEffect(() => {
    listTools()
      ...
      .catch(() => {});
    Promise.all([listProviders(), getModels()])
      .then(([p, m]) => {
        setProviders(p);
        setAllModels(m);
      })
    ...
    listPlugins()
      .then((plugins) => {
        ...
        const messagingPlugins = new Set([
          "whatsapp",
          "telegram",
          "slack",
          "discord",
  ...
  const providerModels = selectedProvider
    ? allModels
        .filter((m) => m.startsWith(`${selectedProvider}:`))
        .map((m) => m.substring(selectedProvider.length + 1))
    : [];
  ...
  const handleSave = () => {
    ...
    if (stepType === "agent") {
      ...
      updated.approval =
        Object.keys(approval).length > 0 ? approval : undefined;
    }
    onSave(updated);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          style={{ fontSize: 12, fontWeight: 600, color: "#e0e0e0", flex: 1 }}
        >
          Edit Step
        </span>
        <button
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "12px" }}>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <textarea
            ...
            placeholder={
              stepType === "approval"
                ? "Pipeline needs your approval to continue.\n\nContext: {{input}}"
                : undefined
            }
            style={{
              ...inputStyle,
              resize: "vertical",
              minHeight: stepType === "approval" ? 60 : 100,
            }}
          />
          ...
          <>
            <div style={{ marginBottom: 12 }}>
              ...
              <select
                value={notifyPlugin}
                onChange={(e) => {
                  setNotifyPlugin(e.target.value);
                  setNotifyChannel("");
                }}
              ...
              <div style={{ fontSize: 9, color: "#444", marginTop: 3 }}>
                {notifyPlugin
                  ? `Send notification via ${notifyPlugin}`
                  : "Approve/reject only from the UI"}
              </div>
            </div>
            {notifyPlugin && (
              <div style={{ marginBottom: 12 }}>
                ...
                >
                  {notifyPlugin === "gmail"
                    ...
                    : notifyPlugin === "slack"
                      ? "Channel or User ID"
                      : notifyPlugin === "discord"
                        ? "Channel or User ID"
                        : notifyPlugin === "telegram"
                          ? "Chat ID"
                          : "Chat / Group ID"}
                </label>
                <input
                  ...
                  placeholder={
                    notifyPlugin === "gmail"
                      ? "user@example.com"
                      : notifyPlugin === "slack"
                        ? "C01ABC123 or U01XYZ789"
                        : notifyPlugin === "discord"
                          ? "1234567890123456"
                          : notifyPlugin === "telegram"
                            ? "-1001234567890"
                            : notifyPlugin === "whatsapp"
                ...
                <div style={{ fontSize: 9, color: "#444", marginTop: 3 }}>
                  {notifyPlugin === "gmail" &&
                    "Email with approval link (approve/reject from browser)"}
                  {notifyPlugin === "slack" &&
                    "Channel ID (C...) for group, User ID (U...) for DM"}
                  {notifyPlugin === "discord" &&
                  ...
                  {![
                    ...
                    "telegram",
                    "whatsapp",
                  ].includes(notifyPlugin) && "Channel or chat identifier"}
                </div>
              </div>
            )}
            <div style={{ marginBottom: 12 }}>
              ...
              />
              <div style={{ fontSize: 9, color: "#444", marginTop: 3 }}>
                Pipeline will fail if not approved within this time (default: 1
                hour)
              </div>
            </div>
          </>
...
function PipelineSettingsSheet({
  ...
  const [triggerType, setTriggerType] = useState<PipelineTrigger["type"]>(
    detail.trigger.type,
  );
  const [schedule, setSchedule] = useState(
    detail.trigger.schedule ?? "every:5m",
  );
  const [webhookSecret, setWebhookSecret] = useState(
    detail.trigger.secret ?? "",
  );
  ...
  const [debounceSecs, setDebounceSecs] = useState(
    String(detail.trigger.debounceSecs ?? 5),
  );
  const [saving, setSaving] = useState(false);
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <span
          style={{ fontSize: 11, fontWeight: 600, color: "#e0e0e0", flex: 1 }}
        >
          {stepRun.stepName}
        </span>
...
function computeTopologicalLevels(
  ...
  for (const c of connections) {
    if (
      c.fromStep === "__trigger__" ||
      !stepIds.has(c.fromStep) ||
      !stepIds.has(c.toStep)
    )
      continue;
...
export default function PipelineFlowView({
  ...
  const handleDeleteStep = useCallback(
    async (stepId: string) => {
      ...
      const incoming = detail.connections
        .filter((c) => c.toStep === stepId)
        .map((c) => c.fromStep);
      const outgoing = detail.connections
        .filter((c) => c.fromStep === stepId)
        .map((c) => c.toStep);
      ...
      for (const from of incoming) {
        for (const to of outgoing) {
          if (
            !newConnections.some((c) => c.fromStep === from && c.toStep === to)
          ) {
  ...
  const handleConnect: OnConnect = useCallback(
    async (connection: Connection) => {
      if (!detail || !pipelineId || !connection.source || !connection.target)
        return;
      // Prevent duplicate connections
      if (
        detail.connections.some(
          (c) =>
            c.fromStep === connection.source && c.toStep === connection.target,
        )
      )
        return;
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (run) {
          setActiveRun(run);
          if (viewingStep) {
            const updated = run.stepRuns.find(
              (s) => s.stepId === viewingStep.stepId,
            );
  ...
  useEffect(() => {
    ...
    if (activeRun)
      for (const sr of activeRun.stepRuns) srMap.set(sr.stepId, sr);
    ...
    const nodes = detail.steps.map((step, idx) => {
      const levelInfo = levels.get(step.id) ?? {
        level: idx,
        indexInLevel: 0,
        levelSize: 1,
      };
      const autoX =
        (levelInfo.indexInLevel - (levelInfo.levelSize - 1) / 2) * 240;
  ...
  const totalTokens = activeRun
    ? activeRun.tokenUsage.input + activeRun.tokenUsage.output
    : 0;
      ...
      >
        ...
        {activeRun && (
          ...
          >
            ...
            {activeRun.completedAt && (
              <span style={{ display: "flex", alignItems: "center", gap: 2 }}>
                <Clock size={9} />{" "}
                {formatElapsed(activeRun.startedAt, activeRun.completedAt)}
              </span>
            )}
            {totalTokens > 0 && (
              <span style={{ display: "flex", alignItems: "center", gap: 2 }}>
                <Coins size={9} /> {formatTokens(totalTokens)}
              </span>
            )}
          </div>
        )}
        ...
        {isRunning ? (
          ...
          <button
            ...
            style={{
              border: "1px solid #333",
              background: triggering ? "transparent" : "#fafafa",
